...
void vtxdistribut::uptime(name account) {
  ...
  // Calculate the time since last user vote
  auto voter = voter_info.find(account.value);
  uint32_t last_vote = voter.date;
  uint32_t time_since_last_vote = now - last_vote;
  check(time_since_last_vote > one_day, "You have not voted for more than one day");
