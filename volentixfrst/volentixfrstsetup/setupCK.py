...
def setContractSteps():
    ...
    try:
        ...
        out = subprocess.check_output(["cleos", '--url', blockchain.producer, 'set', 'code', account.name, '/home/sylvain/eclipse-workspace/ledger/volentixfrst/volentixfrst.wasm'])
        out = subprocess.check_output(["cleos", '--url', blockchain.producer, 'set', 'abi', account.name,  '/home/sylvain/eclipse-workspace/ledger/volentixfrst/volentixfrst.abi'])
    except:
...
def compileContract():
    ...
    out = subprocess.check_output(['/usr/local/eosio.cdt/bin/eosio-cpp', '-o', os.environ['HOME'] + '/eclipse-workspace/ledger/volentixfrst/volentixfrst.wast' , os.environ['HOME'] + '/eclipse-workspace/ledger/volentixfrst/volentixfrst.cpp' ])
    print(str(out))
