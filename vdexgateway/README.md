...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway regaccount '["bob"]' -p bob@active
...
```console
maxpetriev@maxpetriev:~$ cleos get table vdexgateway BVTX accntinfo
...
2. Then Bob send 0.05325221 BTC to `15Ez6SCQF7RA6pXUBbnPijaEmQahFkYdzE` and submit transaction hash to gateway contract.
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway submitdep '[ "bob", "0.05325221 BVTX", "d2cae97da959fc75d0b057765b1ec480b297f0d54d3d6c8c9bd11fb445eb31d2"]' -p bob@active
...
```
   ...
   Bob can claim 0.05325221 BVTX once vdexnodes confirm that deposit transaction `d2cae97da959fc75d0b057765b1ec480b297f0d54d3d6c8c9bd11fb445eb31d2` becomes irreversible in bitcoin blockchain.

3. Vdex node confirm deposit using confirmdep action
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway confirmdep '[ "vdexnode1", "bob", "d2cae97da959fc75d0b057765b1ec480b297f0d54d3d6c8c9bd11fb445eb31d2" ]' -p vdexnode1@active
...
```
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway confirmdep '[ "vdexnode2", "bob", "d2cae97da959fc75d0b057765b1ec480b297f0d54d3d6c8c9bd11fb445eb31d2" ]' -p vdexnode2@active
...
```console
maxpetriev@maxpetriev:~$ cleos get  table vdexgateway bob accntdeposit
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway claimdep '[ "bob", "d2cae97da959fc75d0b057765b1ec480b297f0d54d3d6c8c9bd11fb445eb31d2"]' -p bob@active
...
1. Register account to get deposit address 
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway regaccount '["alice"]' -p alice@active
...
```console
maxpetriev@maxpetriev:~$ cleos push action volentixgsys transfer '[ "alice", "vdexgateway",  "1.00000000 BVTX", "memo" ]' -p alice@active
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway withdraw '["alice", "1.00000000 BVTX", "1Fu3iBR2EMQWeYGi3XvrPmcPUkne8ZWj9h"]' -p alice@active
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway lockwithdraw '["vdexnode1", "alice", "0"]' -p vdexnode1@active
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway submitwithd '["vdexnode1", "alice", "0", "08de200a964d7597cf9e78a69ccfc276cf767c07f97949957ece4af61a1ac37e"]' -p vdexnode1@active
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway confirmwithd '["vdexnode1", "alice", "08de200a964d7597cf9e78a69ccfc276cf767c07f97949957ece4af61a1ac37e"]' -p vdexnode1@active
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway confirmwithd '["vdexnode2", "alice", "08de200a964d7597cf9e78a69ccfc276cf767c07f97949957ece4af61a1ac37e"]' -p vdexnode2@active
...
```
   Now withdraw is confirmed, but vdexnode1 hasn't rewarded yet
```console
maxpetriev@maxpetriev:~$ cleos get  table vdexgateway alice accntwithdra
{
  "rows": [{
      "id": 0,
...
```console
maxpetriev@maxpetriev:~$ cleos push action vdexgateway getreward '["vdexnode1", "alice", "08de200a964d7597cf9e78a69ccfc276cf767c07f97949957ece4af61a1ac37e"]' -p vdexnode1@active
...
```
   Vdexnode got 2 BVTX cause:<br />
      a) Alice withdrawed 1 BTC(1 BTC = 1 BVTX)<br />
      b) reward amount for BVTX withdraw is set to 1 BVTX
```console
maxpetriev@maxpetriev:~$ cleos get  table vdexgateway vdexgateway currency
{
  "rows": [{
