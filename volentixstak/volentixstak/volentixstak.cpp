...
void volentixstak::transfer_liquid_to_staked(name from, name to, asset quantity)
{
   sub_balance( from, quantity);
...
void volentixstak::transfer_staked_to_liquid(name from, name to, asset quantity)
{
   //locked
...
void volentixstak::transfer_staked_to_staked(name from, name to, asset quantity)
{
   sub_balance( from, quantity, true);
   add_balance( to, quantity, from );
...
void volentixstak::inline_stake(name owner, asset quantity, name rampayer)
{
   check( quantity.is_valid(), "invalid quantity" );
...
void volentixstak::stake(name owner, asset quantity)
{
   require_auth( owner );
   inline_stake(owner, quantity, owner); // use owner as payer 
}

void volentixstak::autostake(name owner, asset quantity) 
{
   require_auth( _self );
   inline_stake(owner, quantity, same_payer); // keep payer as before
}

void volentixstak::unstake(name owner, asset quantity)
{
   require_auth( owner );
...
asset volentixstak::collect_refund(name owner, const symbol& symbol, uint64_t auto_index)
{
   refunds_table refunds_tbl( _self, owner.value );
...
void volentixstak::inline_refund(name owner, name rampayer, uint64_t index)
{
   refunds_table refunds_tbl( _self, owner.value );
...
void volentixstak::autorefund(name owner, uint64_t index) 
{
   require_auth( _self );
   inline_refund(owner, same_payer, index); // keep payer as before
}

void volentixstak::refund(name caller, name owner, uint64_t index) 
{
   require_auth( caller );
   inline_refund(owner, same_payer, index); // keep payer as before
}

void volentixstak::cancelunstake(name owner, uint64_t index)
{
   require_auth( owner );
...
void volentixstak::sub_balance( name owner, asset value , bool use_locked_balance) 
{
   accounts from_acnts( _self, owner.value );
...
void volentixstak::add_balance( name owner, asset value, name ram_payer )
{
...
void volentixstak::open( name owner, const symbol& symbol, name ram_payer )
{
...
void volentixstak::close( name owner, const symbol& symbol )
{
   require_auth( owner );
   accounts acnts( _self, owner.value );
...
void volentixstak::setdelay(const symbol& symbol, uint64_t t)
{
   auto sym_code_raw = symbol.code().raw();
...
void volentixstak::settransfee(const symbol& symbol, uint64_t r, name receiver)
{
   auto sym_code_raw = symbol.code().raw();
...
EOSIO_DISPATCH( volentixstak, (execquery)(callback)(create)(issue)(transfer)(open)(close)(retire)(stake)(unstake)(cancelunstake)(refund)(autorefund)(setdelay)(settransfee)(autostake)(addblacklist)(rmblacklist))